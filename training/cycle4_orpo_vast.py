#!/usr/bin/env python3
"""
MiganCore Cycle 4 ORPO Training — Vast.ai Orchestration
=========================================================
Day 63 | 2026-05-07

Dataset  : ~740 pairs (560 curated + 180 new targeted)
           New: evolution_aware×40 + creative×50 + tool_discriminate×50 + voice_natural×40
Algorithm: ORPO — SFT + odds-ratio preference, beta=0.1, lr=6e-7, 2 epochs
Target   : A40/A100 40-80GB @ ~$0.27-0.60/hr | ~20-30 min total
Output   : Tiranyx/migancore-7b-soul-v0.4 (ORPO Cycle 4 adapter)
HF model : Tiranyx/migancore-7b-soul-v0.4

Cycle 4 OKRs (fix Cycle 3 regressions):
  weighted_avg ≥ 0.92   (Cycle 3: 0.9082 — raise bar)
  evolution-aware ≥ 0.80 (Cycle 3: 0.568 ❌ CRITICAL REGRESSION FIX)
  creative ≥ 0.80        (Cycle 3: 0.695 ❌ new category, underrepresented)
  tool-use ≥ 0.85        (Cycle 3: 0.797 — improve)
  voice ≥ 0.85           (Cycle 3: 0.817 — polish)
  identity ≥ 0.94        (Cycle 3: 0.953 ✅ — preserve)

Lessons applied (cumulative Cycle 1–3):
...
DATASET_PATH  = "/opt/ado/data/workspace/cycle4_combined_dataset.jsonl"
TRAIN_SCRIPT  = "/opt/ado/training/train_simpo_standard.py"
OUTPUT_DIR    = "/opt/ado/cycle4_output"
LOG_PATH      = "/tmp/cycle4_training.log"

HF_REPO       = "Tiranyx/migancore-7b-soul-v0.4"
...
COST_CAP_USD      = 5.00  # hard stop (Cycle 3 cost was ~$0.09)

# Training hyperparams (same proven config as Cycle 3 — don't change what works)
# Only change: dataset size 685 → ~740
SIMPO_ARGS = [
    "--dataset",       "/root/cycle4_combined_dataset.jsonl",
    "--output-dir",    "/root/cycle4_adapter",
    "--base-model",    BASE_MODEL,
    "--epochs",        "2",          # same as Cycle 3 (2 epochs converged well)
    "--learning-rate", "6e-7",       # same as Cycle 3
    "--simpo-beta",    "2.5",
    "--simpo-gamma",   "1.0",
    "--loss-type",     "apo_zero",
    "--lora-r",        "16",
...
def main():
    ...
    log("=" * 60)
    log("MIGANCORE CYCLE 4 — ORPO Training (Vast.ai)")
    log(f"Dataset: {DATASET_PATH}")
    log(f"Pairs  : ~740 (560 curated + 180 new targeted)")
    log(f"  New: evolution_aware×40 + creative×50 + tool_discriminate×50 + voice_natural×40")
    log(f"Epochs : 2 (same as Cycle 3)")
    log(f"LR     : 6e-7 (same as Cycle 3)")
    log(f"Target : {HF_REPO}")
    log(f"Goal   : weighted_avg ≥ 0.92, evolution-aware ≥ 0.80, creative ≥ 0.80")
    log(f"         tool-use ≥ 0.85, voice ≥ 0.85, identity ≥ 0.94 (PRESERVE)")
    log("=" * 60)
    ...
    if not Path(DATASET_PATH).exists():
        ...
        log(f"Run export_cycle4_dataset.py first:")
        log(f"  docker compose exec -T api python /app/workspace/export_cycle4_dataset.py")
        sys.exit(1)
    dataset_lines = len(Path(DATASET_PATH).read_text().splitlines())
    log(f"Dataset: {dataset_lines} pairs ✓")
    if dataset_lines < 600:
        log(f"WARNING: only {dataset_lines} pairs — expected ~740. Proceed anyway? (will continue)")
    ...
    ok1 = scp_to(DATASET_PATH, ssh_host, ssh_port, "/root/cycle4_combined_dataset.jsonl")
    ...
    rc, out, _ = ssh(ssh_host, ssh_port,
                     "wc -l /root/cycle4_combined_dataset.jsonl /root/train_simpo_standard.py")
    log(f"Remote file check: {out.strip()}")

    # ── 6. Train ──────────────────────────────────────────────
    log("\n[6/8] Starting ORPO training (Cycle 4: 2 epochs, LR=6e-7, ~740 pairs)...")
    ...
    log("Training started. Estimated 20-35 min for ~740 pairs × 2 epochs on A40/A100...")
    ...
    rc, out, _ = ssh(ssh_host, ssh_port,
                     "ls -lh /root/cycle4_adapter/ 2>/dev/null | head -20")
    log(f"Adapter contents:\n{out}")

    adapter_local = f"{OUTPUT_DIR}/cycle4_adapter"
    ok = scp_from(ssh_host, ssh_port, "/root/cycle4_adapter", adapter_local, timeout=600)
    if not ok:
        log("Adapter download failed. Manual SCP:")
        log(f"  scp -i {SSH_KEY} -r -P {ssh_port} root@{ssh_host}:/root/cycle4_adapter {adapter_local}")
    else:
        log(f"Adapter saved to {adapter_local} ✓")

        log("Uploading adapter to HuggingFace...")
        hf_cmd = (
            f"cd /root/cycle4_adapter && "
            f"pip install -q huggingface_hub && "
            f"huggingface-cli upload {HF_REPO} . "
            f"--token {HF_TOKEN} "
            f"--commit-message 'Day 63: ORPO Cycle 4 (~740 pairs, 2ep, lr=6e-7, evolution+creative+tool_disc+voice fix)' "
            f"2>&1 | tail -10"
        )
    ...
    log(f"Log          : {LOG_PATH}")
    log("=" * 60)
    log("\nNext steps: GGUF conversion + PROMOTE to Ollama")
    log("")
    log("  # 1. Convert to GGUF LoRA")
    log(f"  python3 /opt/llama.cpp/convert_lora_to_gguf.py {adapter_local}/ \\")
    log(f"    --outfile {OUTPUT_DIR}/cycle4_lora.gguf --outtype f16")
    log("")
    log("  # 2. Copy into Ollama volume")
    log(f"  cp {OUTPUT_DIR}/cycle4_lora.gguf /opt/ado/data/ollama/cycle4_lora.gguf")
    log("")
    log("  # 3. Register in Ollama")
    log("  echo 'FROM qwen2.5:7b-instruct-q4_K_M' > /tmp/Modelfile_cycle4")
    log("  echo 'ADAPTER /root/.ollama/cycle4_lora.gguf' >> /tmp/Modelfile_cycle4")
    log("  docker exec ado-ollama-1 ollama create migancore:0.4 -f /root/.ollama/Modelfile_cycle4")
    log("")
    log("  # 4. Run eval vs baseline (PROMOTE gate ≥ 0.92 weighted_avg)")
    log("  docker exec ado-api-1 python /app/eval/run_identity_eval.py \\")
    log("    --model migancore:0.4 --reference /app/eval/baseline_day58.json")
    log("")
    log("  # 5. If PROMOTE: update DEFAULT_MODEL in settings → restart api")
    log("  # If ROLLBACK: stay on migancore:0.3 + debug data mix")
    log("")
